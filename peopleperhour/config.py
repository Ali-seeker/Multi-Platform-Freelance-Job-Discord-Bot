"""
peopleperhour/config.py — Configuration loader and query constants for the PeoplePerHour platform.
"""
...
try:
    ...
    logger.warning(f"Could not load peopleperhour/config.json: {e}. Using defaults.")
    _config_data = {
        "channel_name": "peopleperhour",
        "channel_id": "",
        "tracked_queries": [
            {
                "query": "automation",
                "url": "https://www.peopleperhour.com/freelance-jobs?q=automation&sort=latest",
                "label": "Automation",
            }
        ],
        "fetch_interval": 15,
...
CHANNEL_NAME = _config_data.get("channel_name", "peopleperhour")
CHANNEL_ID = _config_data.get("channel_id", "")
POLL_INTERVAL_SECONDS = _config_data.get("fetch_interval", 15)
JOBS_PER_PAGE = int(_config_data.get("jobs_per_page", os.getenv("PEOPLEPERHOUR_JOBS_PER_PAGE", "20")))
...
def save_config() -> None:
    """Save the in-memory config back to peopleperhour/config.json."""
    ...
    try:
        ...
        logger.warning(f"Could not save peopleperhour/config.json: {e}")


def set_channel_id(new_id: str) -> None:
    """Persist the resolved or created PeoplePerHour Discord channel ID."""
    global CHANNEL_ID
    CHANNEL_ID = str(new_id)
    save_config()
    logger.info(f"💾 Saved PeoplePerHour channel ID ({CHANNEL_ID}) to peopleperhour/config.json")


def add_new_tracker(keyword_or_url: str, label: str = ""):
    """Adds a new query to PeoplePerHour tracking. All jobs route to #peopleperhour."""
    parsed_query = keyword_or_url
    if "peopleperhour.com" in keyword_or_url:
    ...
    for item in TRACKED_QUERIES:
        if item.get("label", "").lower() == label.lower():
            logger.info(f"Query '{label}' is already tracked on PeoplePerHour.")
            return item

    new_entry = {"query": parsed_query, "url": pph_url, "label": label}
    TRACKED_QUERIES.append(new_entry)
    save_config()
    logger.info(f"💾 Added new query to PeoplePerHour tracking: {label}")
    return new_entry


def remove_tracker_by_label(label: str):
    """Removes a query from PeoplePerHour tracking by label."""
    for i, entry in enumerate(TRACKED_QUERIES):
        if entry.get("label", "").lower() == label.lower():
            removed = TRACKED_QUERIES.pop(i)
            save_config()
            logger.info(f"🗑️ Removed query from PeoplePerHour tracking: {label}")
            return removed
    return None
