"""
upwork/formatter.py — Discord embed and thread formatting for Upwork jobs.
"""
...
def format_job_message(
    job: dict, details: dict = None, is_updated: bool = False, query_label: str = ""
) -> tuple[str, discord.Embed]:
    """
    Build a formatted Discord message for a new Upwork job posting.

    Args:
        job: Parsed job dictionary from parse_job()
        details: Optional full details dictionary from fetch_job_details()
        is_updated: Whether this job was updated by client
        query_label: Keyword or search label that matched this job
    ...
    if "entry" in level_lower:
        ...
        color = 0x14A800  # Upwork brand green
